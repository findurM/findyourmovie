...
const LoginPage: React.FunctionComponent<ILoginPageProps> = (props) => {
  ...
  const signInWithGoogle = async () => {
    ...
    signInWithPopup(auth, new GoogleAuthProvider())
      .then((response) => {
        dispatch(isExistUserInfo(response.user?.uid)).then((isExist) => {
          if (!isExist) {
  ...
  return (
    <>
      <div
        className="bg-base-300 min-h-[90vh] flex flex-col sm:flex-row sm:justify-around pt-20"
        style={{
          backgroundImage: 'url("/assets/FindurM_login_hero.jpg")',
            ...
            <form className="mt-8 space-y-6" action="#" method="POST">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="text-sm">
                  <a className="font-medium text-accent hover:text-indigo-500">비밀번호 찾기</a>
                </div>
              </div>
        ...
        <div className="flex-col w-4/12 pt-24 mx-auto text-white">
          <div>
            <p className="text-5xl font-bold">
              Hello, <br /> Welcome Friend!
            </p>
            <p className="mt-10 text-lg">회원가입하고 많은 혜택을 누려보세요!</p>
          </div>
          <Link to="/register">
            <RoundButton className="mx-auto my-10">회원가입</RoundButton>
          </Link>
        </div>
      </div>
      <Footer />
