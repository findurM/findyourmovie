...
const HomePage: React.FunctionComponent<IHomePageProps> = (props) => {
  ...
  useEffect(() => {
    const AuthCheck = onAuthStateChanged(auth, (user) => {
      if (user) {
        dispatch(isExistUserInfo(user.uid)).then((isExist) => {
          if (!isExist) {
  ...
  return (
    <>
      ...
      <label htmlFor="my-modal-6" className="btn modal-button" style={{ display: "none" }}>
        open modal
      </label>
      ...
      <div className="modal modal-bottom sm:modal-middle">
        <div className="modal-box">
          ...
          <InputBox>
            <label htmlFor="age">나이: {age}세</label>
            <input
              type="range"
              min="10"
              max="90"
              value={age}
              step="1"
              className="range range-secondary my-3"
              id="age"
              name="age"
          ...
          <div className="modal-action">
            <label htmlFor="my-modal-6" className="btn" onClick={updateUserInfo}>
              확인
            </label>
          </div>
        </div>
